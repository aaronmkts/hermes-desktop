...
import type { OfficeStatus, OfficeProfileStatus } from "../../../../main/office-status";

const EMPTY_KANBAN = { todo: 0, ready: 0, running: 0, blocked: 0, doneRecent: 0 };
const EMPTY_PLATFORMS = { connected: 0, error: 0, configured: 0 };
const AGENT_COLORS = ["#22c55e", "#38bdf8", "#f59e0b", "#64748b", "#ef4444", "#a855f7", "#0891b2", "#db2777"];
...
function normalizeState(state: unknown): OfficeAgentStatus {
  return ["active", "available", "idle", "offline", "error", "waiting"].includes(String(state))
    ? (state as OfficeAgentStatus)
    : "idle";
}

export function officeStatusToAgents(status: OfficeStatus | null | undefined): OfficeAgent[] {
  return (status?.profiles ?? []).map((profile: Partial<OfficeProfileStatus>) => {
    ...
    const subtitle = profile.model || profile.provider || profile.description || null;
    return {
      id,
