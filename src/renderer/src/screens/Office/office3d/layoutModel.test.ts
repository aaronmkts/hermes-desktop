...
import { buildWorkstations, EXECUTIVE_DECOR, REST_FURNITURE } from "./layout";
import {
  assignDesk,
...
describe("office layout model", () => {
  it("builds a versioned default layout from generated desks and furniture", () => {
    ...
    expect(layout.desks).toEqual(buildWorkstations(["a", "boss"], "boss"));
    expect(layout.furniture.map((p) => p.id)).toEqual(
      [...REST_FURNITURE, ...EXECUTIVE_DECOR].map((p) => p.id),
    );
    expect(
      buildDefaultOfficeLayout(["a"], null).furniture.map((p) => p.id),
  ...
  it("moves furniture and desks immutably while clamping to canvas bounds", () => {
    ...
    expect(layout.furniture[0].x).toBe(1300);
    expect(movedFurniture.furniture[0]).toMatchObject({ x: 0, y: 425 });
    const movedDesk = moveLayoutItem(layout, "desk:desk-0", 10, 20);
    expect(movedDesk.desks[0].deskX).toBe(layout.desks[0].deskX + 10);
    expect(movedDesk.desks[0].chairY).toBe(layout.desks[0].chairY + 20);
    expect(movedDesk.desks[0].seatY).toBe(layout.desks[0].seatY + 20);
  });

  it("rotates furniture in 15 degree increments and desks without losing seat facing", () => {
    const layout = buildDefaultOfficeLayout(["a"], null);
    expect(
      rotateLayoutItem(layout, "furniture:beanbag-0", 17).furniture[0]
        .facingDeg % 15,
    ).toBe(0);
    const rotated = rotateLayoutItem(layout, "desk:desk-0", -15);
    expect(rotated.desks[0].deskFacingDeg).toBe(345);
    expect(rotated.desks[0].chairFacingDeg).toBe(345);
    expect(rotated.desks[0].seatFacing).toBe(layout.desks[0].seatFacing);
  });
  ...
  it("assigns an agent to exactly one desk and supports unassignment immutably", () => {
    ...
    expect(assigned.desks.map((d) => d.agentId)).toEqual([null, "a"]);
    expect(assignDesk(assigned, "desk-1", null).desks[1].agentId).toBeNull();
  });
  ...
  it("normalizes persisted layouts, drops invalid furniture, and appends missing agent desks", () => {
    ...
    expect(normalized.furniture.some((p) => p.id === "bad")).toBe(false);
    expect(normalized.desks.some((d) => d.agentId === "b")).toBe(true);
